    ...
    <div className="min-h-screen bg-[#020810] text-white">
      ...
      <div className="relative z-10 mx-auto w-full max-w-[480px] px-4 pt-1 pb-20">
        <button
          type="button"
          onClick={() => router.back()}
          className="mb-2 inline-flex items-center gap-1.5 rounded-full border border-[#295487] bg-gradient-to-r from-[#0b1730] to-[#10203a] px-3 py-1.5 text-white/90 shadow-[0_10px_24px_rgba(0,0,0,0.22)] transition-all duration-200 hover:-translate-y-0.5 hover:border-blue-500/45 hover:text-white"
          aria-label="Go back"
        >
        ...
        <div className="mb-4 grid grid-cols-3 gap-2">
          {[
            ...
            <div
              key={s.lbl}
              className="relative overflow-hidden rounded-2xl border border-[#0f2244] bg-[#060f22] px-3 py-3 text-center"
            >
        ...
        {/* FILTER TABS */}
        <div className="mb-4 flex gap-1.5 rounded-2xl border border-[#0f2244] bg-[#060f22] p-1">
          {FILTERS.map((f) => (
            <button
              key={f.value}
              type="button"
        ...
        <div className="overflow-hidden rounded-2xl border border-[#0f2244] bg-[#060f22] shadow-2xl">
          ...
          {/* Table Header */}
          <div className="grid grid-cols-[28px_88px_1fr_60px_72px] items-center gap-2 border-b border-[#0f2244]/70 bg-[#04091a]/60 px-4 py-2.5">
            {["#", "Games", "Bet", "Amt", "Status"].map((h, i) => (
              <span
                key={h}
                className={`text-[9px] font-semibold uppercase tracking-widest text-white/25 ${i >= 3 ? "text-right" : ""}`}
              >
                {h}
              </span>
            ))}
            ...
            <div>
              {bets.map((bet: any, idx: number) => {
                ...
                return (
                  <div
                    key={bet.id ?? idx}
                    className="border-b border-[#0a1a38]/60 px-4 py-3 transition-colors last:border-0 hover:bg-blue-500/3"
                    style={{ animationDelay: `${idx * 50}ms` }}
                  >
                    <div className="grid grid-cols-[28px_88px_1fr_60px_72px] items-center gap-2">
                      {/* Serial */}
                      <span className="text-[11px] font-mono font-semibold text-white/25">
                      ...
                      {/* Bet Number */}
                      <div className="min-w-0">
                        <p className="font-mono text-sm font-bold tracking-widest text-white leading-none">
                          {displayBetNumber}
                        </p>
                      </div>
        ...
        {totalPages > 1 && (
          <div className="mt-4 flex items-center justify-between gap-3">
            <button
              ...
              className="flex h-10 items-center gap-1.5 rounded-[14px] border border-[#0f2244] bg-[#060f22] px-4 text-sm font-semibold disabled:opacity-30 hover:border-blue-700/50 transition-colors"
            >
