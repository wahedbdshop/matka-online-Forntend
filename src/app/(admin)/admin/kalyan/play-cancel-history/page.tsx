...
export default function KalyanPlayCancelHistoryPage() {
  const [page, setPage] = useState(1);

  const { data, isLoading } = useQuery({
    queryKey: ["kalyan-cancel-history", page],
    queryFn: () => KalyanAdminService.getCancelHistory({ page, limit: LIMIT }),
  });
  ...
  const getCancelledGameLabel = (row: any) => {
    ...
    return getHistoryValue(row.newValue, [
      "selectedNumber",
      "selectedNumbers",
      "betNumber",
      "betNumbers",
  ...
  const getCancelledAmountLabel = (row: any) => {
    const value = getHistoryValue(row.oldValue, [
      "amount",
      "betAmount",
      "totalAmount",
