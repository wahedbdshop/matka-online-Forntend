  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-violet-100 dark:bg-purple-500/15">
          <Gamepad2 className="h-5 w-5 text-purple-400" />
        </div>
        <div>
          <h1 className="text-xl font-bold text-slate-950 dark:text-white">Lottery Games</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">Manage market names, sessions, and status</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <div className="flex gap-1">
          <input
            ...
            className="w-52 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-900 outline-none placeholder:text-slate-400 focus:border-purple-400 dark:border-slate-600 dark:bg-slate-800 dark:text-white dark:placeholder:text-slate-500 dark:focus:border-purple-500"
          />
          <button
            type="button"
            onClick={() => {
            ...
            className="rounded-lg border border-slate-200 bg-white px-2.5 transition-colors hover:bg-slate-100 dark:border-slate-600 dark:bg-slate-700 dark:hover:bg-slate-600"
          >
          ...
          <tbody>
            {isLoading ? (
              ...
              markets.map((market: any, index: number) => {
                ...
                return (
                <tr key={market.id} className="border-b border-slate-200 transition-colors hover:bg-slate-50 dark:border-slate-700/50 dark:hover:bg-slate-700/20">
                  ...
                  <td className="border-r border-slate-200 px-4 py-3 text-center last:border-r-0 dark:border-slate-700/40">
                    <div className="flex items-center justify-center gap-2">
                      <span className="font-medium text-slate-900 dark:text-white">{getKalyanMarketOptionLabel(market)}</span>
                      <span className={`rounded-full border px-2 py-0.5 text-[10px] font-medium ${(timing?.sessionType ?? market.sessionType) === "CLOSE" ? "border-red-200 bg-red-50 text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-400" : "border-green-200 bg-green-50 text-green-700 dark:border-green-500/30 dark:bg-green-500/10 dark:text-green-400"}`}>
                  ...
                  <td className="border-r border-slate-200 px-4 py-3 text-center last:border-r-0 dark:border-slate-700/40">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        ...
                        className={`inline-flex items-center gap-1 rounded-lg border px-2.5 py-1.5 text-[10px] font-medium transition-colors disabled:opacity-50 ${
                          effectiveStatus === "ACTIVE"
                            ? "border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-400 dark:hover:bg-amber-500/20"
                            : "border-green-200 bg-green-50 text-green-700 hover:bg-green-100 dark:border-green-500/30 dark:bg-green-500/10 dark:text-green-400 dark:hover:bg-green-500/20"
                        }`}
                      >
                      ...
                      <button type="button" onClick={() => { setDeleteTarget(market); setDeleteError(null); }} className="inline-flex items-center gap-1 rounded-lg border border-red-200 bg-red-50 px-2.5 py-1.5 text-[10px] font-medium text-red-700 transition-colors hover:bg-red-100 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-400 dark:hover:bg-red-500/20">
      ...
      {deleteTarget ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4" onClick={(event) => {
          ...
          <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 space-y-4 shadow-[0_20px_60px_rgba(15,23,42,0.16)] dark:border-slate-700 dark:bg-slate-900 dark:shadow-[0_20px_60px_rgba(0,0,0,0.45)]">
            <h2 className="text-sm font-bold text-slate-950 dark:text-white">Delete Lottery Game</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">Are you sure you want to delete <span className="font-semibold text-slate-900 dark:text-white">{deleteTarget.name}</span>? This action cannot be undone.</p>
            {deleteError ? (
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => { setDeleteTarget(null); setDeleteError(null); }} className="flex-1 rounded-lg border border-slate-200 bg-white py-2 text-sm text-slate-600 transition-colors hover:bg-slate-100 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">Cancel</button>
