...
function StatCard({
  ...
  return (
    <div className="rounded-[24px] border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-950 md:px-4 md:py-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
            {label}
          </p>
          <p className="mt-1.5 text-xl font-bold leading-none text-slate-950 dark:text-white">
            {value}
          </p>
...
function WithdrawalsHistoryContent() {
  ...
  const openPicker = () => {
    const input = selectedDateRef.current;
    if (!input) return;
    input.focus();
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-4 xl:grid-cols-3">
        <StatCard
          label={`${statsDateLabel} Total Withdrawals`}
          value={formatMoney(stats?.totalWithdrawalAmount)}
          hint="Amount requested on selected date"
          icon={Wallet}
          accent="border-cyan-500/20 bg-cyan-500/10 text-cyan-700 dark:text-cyan-300"
        />
        <StatCard
          label={`${statsDateLabel} Pending Withdrawals`}
          value={String(stats?.pendingWithdrawalCount ?? 0)}
        ...
        <StatCard
          ...
          value={String(stats?.totalWithdrawalCount ?? 0)}
          hint="Total records on selected date"
          icon={CreditCard}
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,220px)_minmax(0,190px)_auto]">
        <label className="relative">
          ...
          <input
            ...
            placeholder="Search by user name, account number, transaction ID"
        ...
        <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 dark:border-slate-800 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Showing {selectedDate} withdrawals, {total} records found
            </p>
          </div>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={11} className="px-4 py-14 text-center text-sm text-slate-500 dark:text-slate-400">
                    No matching withdrawal history found
                  </td>
                </tr>
              ) : (
                withdrawals.map((withdrawal: any, index: number) => {
                  const method = getMethodMeta(withdrawal.paymentMethod);
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${STATUS_STYLES[withdrawal.status] ?? STATUS_STYLES.PENDING}`}>
                          {withdrawal.status}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <Link
                          href={`/admin/withdrawals/history/${withdrawal.id}`}
        ...
        <div className="grid gap-3 p-4 xl:hidden">
          ...
          {!isLoading && !isError
            ? withdrawals.map((withdrawal: any, index: number) => {
                ...
                return (
                  <div key={withdrawal.id} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-slate-950 dark:text-white">
                          {rowNumber}. {withdrawal.user?.name ?? "-"}
                        </p>
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-2">
                      {[
                        ["Phone Number", withdrawal.user?.phone ?? "-"],
                        ["Withdrawal Amount", formatMoney(withdrawal.amount)],
                        ["Account Number", withdrawal.accountNumber ?? "-"],
                        ["Transaction ID", withdrawal.transactionId ?? "-"],
                        ["Withdrawal Date & Time", formatDateTime(withdrawal.createdAt)],
                      ].map(([label, value]) => (
