...
function getSection(text: string, startPattern: RegExp, endPattern?: RegExp) {
  ...
  const sliced = text.slice(sliceStart);
...
function extractRecentBlocks(text: string) {
  const blocks = text.match(
    /Government lottery results \d{2} [A-Za-z]+ \d{4}[\s\S]*?(?=Government lottery results \d{2} [A-Za-z]+ \d{4}|Copyright ©|$)/g,
  );
...
function parseBlock(block: string) {
  const dateMatch = block.match(/Government lottery results (\d{2} [A-Za-z]+ \d{4})/);
  if (!dateMatch) {
    return null;
  }
...
export function parseThaigerThaiLottery(html: string) {
  ...
  const recentSection = getSection(flattened, /Government lottery results \d{2} [A-Za-z]+ \d{4}/i);
  const recentBlocks = extractRecentBlocks(recentSection);
  const recentResults = recentBlocks
    .map(parseBlock)
